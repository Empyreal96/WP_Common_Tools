		...
		private static List<RegValueInfo> RemoveDuplicates(List<RegValueInfo> values)
		{
			...
			foreach (RegValueInfo value2 in values)
			{
				if (value2.ValueName != null)
				{
					...
					{
						TraceLogger.LogMessage(TraceLevel.Warn, $"Registry conflict discovered: keyName: {value2.KeyName}, valueName: {value2.ValueName}, oldValue: {value.Value}, newValue: {value2.Value}");
