...
namespace USB_Test_Infrastructure
{
	public class DTSFUsbStream : Stream
	{
		...
		public DTSFUsbStream(string deviceName)
		{
			...
			try
			{
				deviceHandle = CreateDeviceHandle(this.deviceName);
				if (deviceHandle.IsInvalid)
				{
					throw new IOException($"Handle for {deviceName} is invalid.");
				}
				InitializeDevice();
				if (!ThreadPool.BindHandle(deviceHandle))
				{
					throw new IOException($"BindHandle on device {deviceName} failed.");
				}
				Connect();
			}
			catch (Exception)
		...
		{
			ControlTransferSetData(UsbControlRequest.Connect, 1);
		}
	}
}
