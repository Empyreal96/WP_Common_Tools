...
namespace Microsoft.WindowsPhone.WPImage
{
    internal class DismountCommand : IWPImageCommand
    {
        ...
        //public static readonly string IsUnlockedOption = "unlockedImage";
        ...
        // Set this to false if not an unlocked image, TODO: Add cmd args for -unlockedImage
        private bool _unlockedImage = true;

        private IULogger _logger = new IULogger();

        FullFlashUpdateImage.FullFlashUpdateStore updateStore;
        ...
        public void Run()
        {
            ...
            try
            {
               ...
               /* if (_unlockedImage == true)
                {
                    try
                    {
                        FullFlashUpdateImage.FullFlashUpdatePartition updatePartition = new FullFlashUpdateImage.FullFlashUpdatePartition();
                        updatePartition.Name = "BACKUP_BS_NV";
                        updatePartition.PartitionType = "{9146aaec-5f73-4d6a-876f-a8e3c84d8788}";
                        updatePartition.TotalSectors = 0x200;
                        updatePartition.SectorsInUse = 0x0;

                        fullFlashUpdateImage.Stores[0].Partitions.Add(updatePartition);
                ...
                // Double checking Partitions in the FFU manifest
                fullFlashUpdateImage.DisplayImageInformation(_logger);
                //
            ...
            {
                _logger.LogError("Failed to dismount the image: {0}\n{1}\n{2}", innerException.ToString(), innerException.StackTrace, innerException.Source);
