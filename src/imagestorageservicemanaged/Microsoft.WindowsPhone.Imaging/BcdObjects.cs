...
namespace Microsoft.WindowsPhone.Imaging
{
	public static class BcdObjects
	{
		...
		public static Guid IdFromName(string objectName)
		{
			...
			if (guid == Guid.Empty)
			{
				throw new ImageStorageException($"{MethodBase.GetCurrentMethod().Name}: '{objectName}' doesn't have an associated Id.");
			}
			return guid;
		}
		...
		public static uint ObjectTypeFromName(string objectName)
		{
			...
			if (num == 0)
			{
				throw new ImageStorageException($"{MethodBase.GetCurrentMethod().Name}: '{objectName}' doesn't have a known type.");
			}
			return num;
		}
	}
