...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	public class OfflineRegUtils
	{
		...
		public static IntPtr CreateKey(IntPtr handle, string keyName)
		{
			...
			{
				string text = array[i];
				int num = OffRegNativeMethods.ORCreateKey(handle, keyName, null, 0u, null, ref keyHandle, ref dwDisposition);
				if (num != 0)
				{
					throw new Win32Exception(num);
				}
		...
		public static int GetVirtualFlags(IntPtr handle)
		{
			...
			int pbFlags = 0;
			int num = OffRegNativeMethods.ORGetVirtualFlags(handle, ref pbFlags);
			if (num != 0)
			{
				throw new Win32Exception(num);
			}
