...
namespace Microsoft.WindowsPhone.ImageUpdate.Tools.Common
{
	public abstract class ResourceAcl
	{
		...
		public string ExplicitDACL
		{
			get
			{
				if (!m_explicitDaclProcessed)
				{
					m_explicitDaclProcessed = true;
					if (m_nos != null)
					{
		...
		public string ElementID
		{
			get
			{
				if (m_elementId == null && !string.IsNullOrEmpty(m_path))
				{
		...
		public string Protected
		{
			...
			set
			{
				m_isProtected = (value.Equals("Yes", StringComparison.OrdinalIgnoreCase) ? true : false);
			}
		}
		...
		public bool IsEmpty
		{
			get
			{
				if (!m_isEmptyProcessed)
				{
					...
					m_isEmpty = string.IsNullOrEmpty(ExplicitDACL) && string.IsNullOrEmpty(MandatoryIntegrityLabel) && !DACLProtected;
				}
				return m_isEmpty;
			}
		}
