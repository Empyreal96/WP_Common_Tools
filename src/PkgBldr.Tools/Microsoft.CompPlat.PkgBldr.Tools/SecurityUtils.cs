...
namespace Microsoft.CompPlat.PkgBldr.Tools
{
	public class SecurityUtils
	{
		...
		public static AclCollection GetFileSystemACLs(string rootDir)
		{
			...
			{
				throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Directory {0} does not exist", new object[1] { rootDir }));
			}
		...
		public static AclCollection GetRegistryACLs(string hiveRoot)
		{
			...
			{
				throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Directory {0} does not exist", new object[1] { hiveRoot }));
			}
			AclCollection aclCollection = new AclCollection();
			foreach (SystemRegistryHiveFiles value in Enum.GetValues(typeof(SystemRegistryHiveFiles)))
			{
				string hivefile = LongPath.Combine(hiveRoot, Enum.GetName(typeof(SystemRegistryHiveFiles), value));
