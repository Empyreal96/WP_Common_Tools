...
namespace Microsoft.CompPlat.PkgBldr.Tools
{
	public static class RegistryUtils
	{
		...
		public static void ConvertSystemHiveToRegFile(DriveInfo systemDrive, SystemRegistryHiveFiles hive, string outputRegFile)
		{
			...
			ConvertHiveToRegFile(LongPath.Combine(LongPath.Combine(systemDrive.RootDirectory.FullName, "windows\\system32\\config"), Enum.GetName(typeof(SystemRegistryHiveFiles), hive)), MapHiveToMountPoint(hive), outputRegFile);
		}
		...
		{
			string args = string.Format(CultureInfo.InvariantCulture, "LOAD {0} {1}", new object[2] { mountpoint, inputhive });
		...
		{
			string args = string.Format(CultureInfo.InvariantCulture, "EXPORT {0} {1}", new object[2] { mountpoint, outputfile });
			string command = Environment.ExpandEnvironmentVariables("%windir%\\System32\\REG.EXE");
			int num = CommonUtils.RunProcess(Environment.CurrentDirectory, command, args, true);
			if (0 < num)
			{
		...
		{
			string args = string.Format(CultureInfo.InvariantCulture, "UNLOAD {0}", new object[1] { mountpoint });
			string command = Environment.ExpandEnvironmentVariables("%windir%\\System32\\REG.EXE");
			int num = CommonUtils.RunProcess(Environment.CurrentDirectory, command, args, true);
			if (0 < num)
			{
		...
		public static string MapHiveFileToMountPoint(string hiveFile)
		{
			...
			if (!hiveMap.TryGetValue(LongPath.GetFileName(hiveFile), out value))
			{
				return "";
			}
